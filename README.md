...
| [`apps/typescript/herdrelay`](apps/typescript/herdrelay/) | TypeScript / Next.js | Human-approved livestock incident coordination: a synthetic barn respiratory alert becomes one CALL-E call to one authorized caretaker, placed only against a SHA-256 fingerprint of the exact call the operator read. The result schema gives every human-answerable field a way to say "they did not say", and any outcome that contradicts its own fields or the transcript is downgraded to uncertain with human review required. It diagnoses nothing, contacts no veterinarian, never redials and never schedules. Dry run by default. |
...
- [`Keeping uncertainty`](apps/typescript/herdrelay/docs/keeping-uncertainty.md) - Giving a result schema somewhere to put "they did not say", so an unstated answer is never rounded to no, and the fail-closed rules that downgrade an outcome contradicting its own fields or its transcript.
